...
import { FiGrid, FiTool, FiShoppingBag, FiFileText, FiInfo, FiBriefcase, FiCalendar, FiFile, FiChevronLeft, FiChevronRight, FiLogOut } from 'react-icons/fi';
...
interface AdminSidebarProps { className?: string; }
    ...
    <aside className={`sidebar admin-sidebar ${collapsed ? 'collapsed' : ''}`}>
      ...
      {!collapsed && (
        <div className="sidebar-user">
          <img src={user?.avatarUrl || user?.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.fullName || user?.firstName || 'Admin')}&background=1a5cff&color=ffffff&bold=true`} alt={user?.fullName} className="sidebar-user-avatar" />
      ...
      <div className="sidebar-footer">
        <NavLink to="/" className="sidebar-link" title={collapsed ? 'Xem trang web' : undefined}>
